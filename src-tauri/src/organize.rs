...
//! a SEPARATE `Library/` folder kept apart from the loose `Downloads/` staging shelf. Raw
//! torrents land under `Downloads/`; this pass walks the storage root (finding both that
//! unsorted shelf and any older loose files) and lifts them into the organized `Library/`.
//! Because each file is finished before the next is touched and organized files leave the
//! source tree, a crash or stop loses no work: the next run re-scans the source (skipping
//! `Library/`, but re-nesting any music still sitting flat in `Music/`) and resumes where it
//! left off. Safety: never deletes a file, never overwrites an existing target, only ever
//! removes *empty* leftover folders (but keeps the `Downloads/` staging folder itself).
//! Falls back to the regex parser when Ollama is offline.
...
pub async fn run(
    ...
    for (idx, f) in files.iter().enumerate() {
        ...
        let (mut to_rel, media): (String, String) = if f.kind == "audio" {
            ...
            // Clean the messy repack/folder name into a real game title (LLM, regex
            // fallback) — "Cyberpunk.2077.v2.1.FitGirl.Repack" → "Cyberpunk 2077" — then
            // collapse spelling variants so every copy lands under one Games/<Title> folder.
            ...
            let title = canonicalize_title(&raw_title, &organized_root.join("Games"), &mut canon);
            // A repack/install is a FOLDER → move the whole thing under Games/<Title>/. A
            // single ROM / disc image is a FILE → Games/<Title>/<Title>.<ext>.
            let rel = if from.is_dir() {
                let t = if title.is_empty() { "Unknown".to_string() } else { title };
                format!("Games/{t}")
            } else {
                build_rel(&title, parsed.as_ref(), f, &ext, "game")
            };
            (rel, "game".to_string())
        } else {
            let stem = f.file_name.strip_suffix(&format!(".{ext}")).unwrap_or(&f.file_name);
        ...
        let (status, message): (&str, Option<String>) = if to.exists() {
            ...
            // .exists() (not .is_file()) so game/app FOLDERS — repacks, ROM sets — move too.
            result.errors += 1;
...
fn sweep_empty_dirs_src(root: &Path, organized_root: &Path) {
    ...
    dirs.sort_by_key(|d| std::cmp::Reverse(d.components().count()));
    // Keep the `Downloads/` staging folder itself as a stable sibling of the library, even
    // once it's drained — fresh torrents download into it, so removing/recreating it on
    // every cleanup would needlessly churn the filesystem watcher.
    let downloads = root.join("Downloads");
    for d in dirs {
        if d == root || d == organized_root || d.starts_with(organized_root) || d == downloads {
            continue;
        }
        let _ = std::fs::remove_dir(&d); // no-op if not empty
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn organizes_a_game_repack_folder() {
        use std::fs;
        let base = std::env::temp_dir().join(format!("bp-game-{}", std::process::id()));
        let _ = fs::remove_dir_all(&base);
        // A repack is a FOLDER (the bug: the move block rejected non-files → "source missing").
        let game = base.join("Cyberpunk 2077 FitGirl Repack");
        fs::create_dir_all(&game).unwrap();
        fs::write(game.join("setup.exe"), vec![0u8; 1024]).unwrap();
        fs::write(game.join("data.bin"), vec![0u8; 4096]).unwrap();

        let organized = base.join("Library");
        let client = reqwest::Client::new();
        // model=None → no network; the game branch falls back to the regex-cleaned title.
        let res = run(&base, &organized, &client, None, true, |_| {}).await;

        let diag: Vec<String> = res
            .moves
            .iter()
            .map(|m| format!("{} {} → {} ({})", m.status, m.from_name, m.to_rel, m.message.clone().unwrap_or_default()))
            .collect();
        assert_eq!(res.errors, 0, "the folder should move, not error: {diag:?}");
        assert!(res.moved >= 1, "the repack folder was organized");
        let games = organized.join("Games");
        let dirs: Vec<String> = fs::read_dir(&games)
            .unwrap()
            .flatten()
            .filter(|e| e.path().is_dir())
            .map(|e| e.file_name().to_string_lossy().into_owned())
            .collect();
        assert_eq!(dirs.len(), 1, "one game folder under Library/Games/: {dirs:?}");
        assert!(games.join(&dirs[0]).join("setup.exe").exists(), "the repack's contents came along");
        // The source repack folder is gone from the download root.
        assert!(!game.exists(), "the original repack folder was moved out");
        let _ = fs::remove_dir_all(&base);
    }
}

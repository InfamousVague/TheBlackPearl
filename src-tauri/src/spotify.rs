...
async fn fetch_playlist_embed(client: &reqwest::Client, id: &str) -> Result<(String, Vec<Track>), String> {
    ...
    let marker = html.find("__NEXT_DATA__").ok_or("This playlist isn't public, so its tracks can't be read.")?;
    let json_start = html[marker..].find('>').map(|i| marker + i + 1).ok_or("Couldn't parse the Spotify embed.")?;
    let json_end = html[json_start..].find("</script>").map(|i| json_start + i).ok_or("Couldn't parse the Spotify embed.")?;
    ...
    let list = find_key(&data, "trackList")
        ...
        .ok_or("This playlist isn't public, so its tracks can't be read.")?;

...
/// Read embedded `(title, artist, album)` tags from an audio file, best-effort. Public so the
/// engine can attach REAL music metadata to a share it offers a friend — the peer then
/// resolves the album cover by artist + album instead of guessing from a bare file name
/// (which is spotty for short titles with no artist, e.g. a single track named "Demons").
pub fn read_tags(path: &Path) -> (Option<String>, Option<String>, Option<String>) {
    read_existing(path)
}

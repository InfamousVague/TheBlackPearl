...
/** Result of importing a shared sources file. */
...
/** Write every configured source to `path` as a portable JSON file; resolves to the count. */
export function exportSources(path: string): Promise<number> {
  return invoke<number>("export_sources", { path });
}

/** Read a sources file at `path` and add any new sources (deduped by URL). */
export function importSources(path: string): Promise<SourcesImportResult> {
  return invoke<SourcesImportResult>("import_sources", { path });
}
...
/** Liveness of GhostWire artwork relay (posters/album art). */
...
export interface BackendPerfEvent {
  at: number;
  name: string;
  durationMs: number;
  detail: string;
...
export interface BackendPerfSnapshot {
  cacheHits: number;
  cacheMisses: number;
  scanRuns: number;
  avgScanMs: number;
  ...
  events: BackendPerfEvent[];
}

export interface BackendScanBench {
  iterations: number;
  itemCount: number;
  minMs: number;
  avgMs: number;
...
export function perfBackendSnapshot(): Promise<BackendPerfSnapshot> {
  if (!IN_TAURI) {
  ...
  return invoke<BackendPerfSnapshot>("perf_backend_snapshot");
}

export function perfBackendClear(): Promise<void> {
  if (!IN_TAURI) return Promise.resolve();
  return invoke("perf_backend_clear").then(() => undefined);
}

export function perfBackendScanBench(iterations = 4): Promise<BackendScanBench> {
  if (!IN_TAURI) {
    return Promise.resolve({
      iterations: 0,
      itemCount: 0,
  ...
  return invoke<BackendScanBench>("perf_backend_scan_bench", { iterations });
}
...
/** VPN kill-switch: halt all download traffic (the VPN dropped while the app is open). */
export function networkPauseAll(): Promise<void> {
  return invoke("network_pause_all").then(() => undefined);
}

/** Lift the VPN kill-switch and let downloads resume. */
export function networkResumeAll(): Promise<void> {
  return invoke("network_resume_all").then(() => undefined);
}

/** Fires when the VPN switches off mid-session; payload is the (now inactive) VPN status. */
export function onVpnDropped(callback: (status: VpnStatus) => void): Promise<() => void> {
  return listen<VpnStatus>("vpn://dropped", (e) => callback(e.payload));
}

// --- Music imports (persistent Spotify/music-link download queue) ---
...
/** True when a pasted string is a music-service link we should import (not a torrent/search). */
export function isMusicImportLink(value: string): boolean {
  const v = value.trim().toLowerCase();
  if (!v) return false;
  if (v.startsWith("magnet:")) return false;
  return (
...
/** Queue a music link (playlist/album/artist/track) for background download. */
export function enqueueMusicImport(url: string): Promise<MusicImportJob> {
  return invoke<MusicImportJob>("music_import_enqueue", { url });
}

export function listMusicImports(): Promise<MusicImportJob[]> {
  return invoke<MusicImportJob[]>("music_imports_list");
}

export function removeMusicImport(id: string): Promise<void> {
  return invoke("music_import_remove", { id }).then(() => undefined);
}

export function retryMusicImport(id: string): Promise<void> {
...
export interface DownloadedItem {
  id: string;
  title: string;
  /** Optional cleaned title from AI/enrichment (when available). */
  cleanTitle?: string | null;
  fileName: string;
...
export async function resolveLocalPlayUrl(item: DownloadedItem): Promise<string> {
  ...
  // The scan baked `url` by extension; for local video, re-resolve by probing the actual
  // codecs so an HEVC/AC-3 ".mp4" routes through the transcoder instead of failing raw.
  if (IN_TAURI && item.kind === "video") {
    return invoke<string>("local_play_url", { rel: item.id }).catch(() => item.url);
  }
  return item.url;
}
...
/** A file flagged by the safety scan as potentially unsafe. */
export interface RiskyFile {
  path: string;
  name: string;
  category: "disguised" | "executable" | "script";
  reason: string;
  danger: boolean;
}

/** Result of scanning a download for risky content (disguised/executable files). */
...
/** Scan a downloaded file/folder (by its relative-path id) for disguised or executable files.
 *  Local + filename-only — instant and private. */
export function scanSafety(id: string): Promise<SafetyReport> {
  return invoke<SafetyReport>("scan_safety", { id });
}
...
/** Download a direct HTTP(S) file into GhostWire's download folder (desktop only flow). */
export function downloadHttpFile(url: string, title?: string, allowDuplicate?: boolean): Promise<string> {
  return invoke<string>("download_http_file", {
    url,
    title: title ?? null,

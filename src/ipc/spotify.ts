...
/** Fetch Spotify album tracks from the public embed for import queue previews. */
export function spotifyAlbumPreview(albumUrl: string): Promise<SpotifyPlaylistPreviewResult> {
  return invoke<SpotifyPlaylistPreviewResult>("spotify_album_preview", { albumUrl });
}

/** Fetch an artist's top tracks from the public embed for import queue previews. */
export function spotifyArtistTopTracksPreview(artistUrl: string): Promise<SpotifyPlaylistPreviewResult> {
  return invoke<SpotifyPlaylistPreviewResult>("spotify_artist_top_tracks_preview", { artistUrl });
}
